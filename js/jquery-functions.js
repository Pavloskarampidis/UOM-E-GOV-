$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε το δελτίο Ανεργίας για το έτος 2025!"
        : "We are sorry but you are not entitled to the unemployment card for 2025!";
  ...
  function retrieveAnswers() {
    ...
    if (
      ...
      getEvidencesById(10);
      currentLanguage === "greek"
        ? setResult("Δικαιούται και ο συνοδός το ίδιο δελτίο ανεργίας 2025.")
        : setResult("The companion is also entitled with the same unemployment card.");
    }
    ...
    if (allAnswers[7] === "1") {
      ...
      currentLanguage === "greek"
      ? setResult(
          "Δικαιούστε την γενική κάρτα ανεργίας για το έτος 2025"
        )
      : setResult(
          "You are entitled to the general unemployment card for 2025"
        );
      ...
      if (allAnswers[8] === "1") {
        currentLanguage === "greek"
          ? setResult(
              "Δικαιούσαι την γενική κάρτα ανεργίας για το έτος 2025"
            )
          : setResult(
              "You are entitled to the general unemployment card for 2025"
            );
      ...
      currentLanguage === "greek"
      ? setResult(
          "Δικαιούσαι την κάρτα ανεργίας για το έτος 2025"
        )
      : setResult(
          "You are entitled to the general unemployment card for 2025"
        );
  ...
  function submitForm() {
    ...
    const titleText =
      currentLanguage === "greek"
        ? "Είστε δικαιούχος της κάρτας ανεργίας!"
        : "You are eligible!";
    ...
    currentLanguage === "greek"
      ...
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>The documents you need to provide in order to receive your unemployment card are the following:</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 3) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Μπορείτε να το εκδώσετε μόνο εάν έχετε απολυθεί ή έχει λήξει η σύμβαση εργασίας σας.") : skipToEnd("You can reissue it only one time in one year.");
        ...
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Πρέπει να έχετε ολοκληρώσει τις στρατιωτικές σας υποχρεώσεις και να είστε ικανός προς εργασία, και σε αναζήτηση.") : skipToEnd("You must have fullfiled military obligations, searching and being able for work");
      } else {
